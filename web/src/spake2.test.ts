...
import { start, finish, deriveW } from "./spake2";

// Official SPAKE2 test vectors from RFC 9382 Appendix B
// (ciphersuite P256-SHA256-HKDF-HMAC; w, x, y supplied directly).
// https://www.rfc-editor.org/rfc/rfc9382#appendix-B
interface Vector {
  A: string;
  B: string;
  w: string;
...
const VECTORS: Vector[] = [
  {
    ...
    Aconf: "58ad4aa88e0b60d5061eb6b5dd93e80d9c4f00d127c65b3b35b1b5281fee38f0",
    Bconf: "d3e2e547f1ae04f2dbdbf0fc4b79f8ecff2dff314b5d32fe9fcef2fb26dc459b",
  },
  {
    A: "",
    B: "client",
    ...
    Aconf: "47d29e6666af1b7dd450d571233085d7a9866e4d49d2645e2df975489521232b",
    Bconf: "3313c5cefc361d27fb16847a91c2a73b766ffa90a4839122a9b70a2f6bd1d6df",
  },
  {
    A: "server",
    B: "",
    ...
    Aconf: "bc9f9bbe99f26d0b2260e6456e05a86196a3307ec6663a18bf6ac825736533b2",
    Bconf: "c2370e1bf813b086dff0d834e74425a06e6390f48f5411900276dcccc5a297ec",
  },
  {
    A: "",
    B: "",
    ...
    Aconf: "dfb4db8d48ae5a675963ea5e6c19d98d4ea028d8e898dad96ea19a80ade95dca",
    Bconf: "d0f0609d1613138d354f7e95f19fb556bf52d751947241e8c7118df5ef0ae175",
  },
];
...
const id = (s: string): Uint8Array => new TextEncoder().encode(s);

describe("SPAKE2 RFC 9382 Appendix B test vectors (P-256)", () => {
  VECTORS.forEach((v, i) => {
    it(`vector ${i + 1}: A='${v.A}' B='${v.B}'`, () => {
      const w = BigInt("0x" + v.w);
      const idA = id(v.A);
      const idB = id(v.B);

      // Message generation with the RFC's fixed scalars.
      const a = start("A", w, idA, idB, undefined, BigInt("0x" + v.x));
      const b = start("B", w, idA, idB, undefined, BigInt("0x" + v.y));
      expect(bytesToHex(a.message)).toBe(v.pA);
      expect(bytesToHex(b.message)).toBe(v.pB);

      // Both sides finish and must agree on the whole key schedule.
      const ra = finish(a.state, b.message);
      const rb = finish(b.state, a.message);

      expect(bytesToHex(ra.ke)).toBe(v.Ke);
      expect(bytesToHex(ra.ka)).toBe(v.Ka);
      expect(bytesToHex(ra.kcA)).toBe(v.KcA);
      expect(bytesToHex(ra.kcB)).toBe(v.KcB);
      expect(bytesToHex(ra.cA)).toBe(v.Aconf);
      expect(bytesToHex(ra.cB)).toBe(v.Bconf);

      // Shared secret matches across parties.
      expect(bytesToHex(rb.ke)).toBe(v.Ke);

      // Key confirmation succeeds with the correct peer MAC.
      expect(ra.verifyPeer(hexToBytes(v.Bconf))).toBe(true);
      expect(rb.verifyPeer(hexToBytes(v.Aconf))).toBe(true);
    });
  });
});

describe("SPAKE2 end-to-end (random scalars) + key confirmation", () => {
  it("two parties using the same code derive the same key and confirm", () => {
    const salt = id("handshake-salt");
    const w = deriveW("ABCDE-FGHJK", salt);
    const idA = id("owner");
    const idB = id("joiner");

    const a = start("A", w, idA, idB);
    const b = start("B", w, idA, idB);
    const ra = finish(a.state, b.message);
    const rb = finish(b.state, a.message);

    expect(bytesToHex(ra.ke)).toBe(bytesToHex(rb.ke));
    expect(ra.verifyPeer(rb.confirm)).toBe(true);
    expect(rb.verifyPeer(ra.confirm)).toBe(true);
  });

  it("different codes fail key confirmation (and derive different keys)", () => {
    const salt = id("handshake-salt");
    const a = start("A", deriveW("RIGHT-CODE0", salt), id("owner"), id("joiner"));
    const b = start("B", deriveW("WRONG-CODE0", salt), id("owner"), id("joiner"));
    const ra = finish(a.state, b.message);
    const rb = finish(b.state, a.message);

    expect(bytesToHex(ra.ke)).not.toBe(bytesToHex(rb.ke));
    expect(ra.verifyPeer(rb.confirm)).toBe(false);
    expect(rb.verifyPeer(ra.confirm)).toBe(false);
  });
});
